...
class AdaDist(nn.Module):
    ...
    def forward(self, texts, training_module=True):
        ...
        if not training_module:
            # Batch all 4 text groups into one forward pass (~4x faster eval).
            all_texts = list(original_text) + list(sampled_text) + original_rewrite_text + sampled_rewrite_text
            tokenized = self.scoring_tokenizer(all_texts, return_tensors="pt", padding=True).to(self.device)
            labels  = tokenized.input_ids[:, 1:]
            lengths = tokenized.attention_mask.sum(dim=1).clamp(min=1).float()
            with torch.no_grad():
                logits = self.scoring_model(
                    input_ids=tokenized.input_ids,
                    attention_mask=tokenized.attention_mask,
                ).logits[:, :-1, :]
            logp = get_logp_statistics(logits, labels, pad_id) / lengths
            B = len(original_text)
            train_original_crit       = logp[:B]
            train_sampled_crit        = logp[B:2*B]
            train_original_regen_crit = logp[2*B:3*B]
            train_sampled_regen_crit  = logp[3*B:]
        else:
            tokenized = self.scoring_tokenizer(list(sampled_text), return_tensors="pt", padding=True).to(self.device)
            labels = tokenized.input_ids[:, 1:]
            train_sampled_crit = self.get_logp(tokenized, labels, pad_id, training_module=True)

            tokenized = self.scoring_tokenizer(list(original_text), return_tensors="pt", padding=True).to(self.device)
            labels = tokenized.input_ids[:, 1:]
            train_original_crit = self.get_logp(tokenized, labels, pad_id, training_module=True)

            tokenized = self.scoring_tokenizer(original_rewrite_text, return_tensors="pt", padding=True).to(self.device)
            labels = tokenized.input_ids[:, 1:]
            train_original_regen_crit = self.get_logp(tokenized, labels, pad_id, training_module=True)

            tokenized = self.scoring_tokenizer(sampled_rewrite_text, return_tensors="pt", padding=True).to(self.device)
            labels = tokenized.input_ids[:, 1:]
            train_sampled_regen_crit = self.get_logp(tokenized, labels, pad_id, training_module=True)
        ...
        # Per-sample crits (not collapsed) so evaluate_model can collect them correctly.
        train_original_crit = train_original_crit - train_original_regen_crit  # [B]
        train_sampled_crit  = train_sampled_crit  - train_sampled_regen_crit   # [B]

        output = dict(crit=[train_original_crit.detach(), train_original_crit,
                             train_sampled_crit.detach(),  train_sampled_crit], loss=MMDloss)
        return output
